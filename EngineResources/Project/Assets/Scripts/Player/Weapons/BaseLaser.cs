...
public class BaseLaser 
{
	...
	{
		TheConsole.Log("Base Laser Shoot Press");
		if(shoot_timer <= 0.0f)
		{
			TheConsole.Log("Base Laser Shoot Timer out");
			//shoot
			...
			{
				TheConsole.Log("Base Laser Shoot has factory");
				TheGameObject go = laser_factory.Spawn();
				
				if(go != null)
				{
					TheConsole.Log("Base Laser Shoot laser created");
					...
					{
						TheConsole.Log("Base Laser Shoot laser info");
