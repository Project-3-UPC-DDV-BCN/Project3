...
Particle::Particle(ComponentParticleEmmiter * parent)
{
	...
	velocity_interpolation_started = false; 
...
void Particle::UpdateVelocity()
{
	...
	if (time > total_time)
		particle_data->change_velocity_interpolation = false; 
}
...
void Particle::Update()
{
	...
	if (particle_timer.Read() >= particle_data->v_interpolation_start_time * 1000 && velocity_interpolation_started == false)
	{
		velocity_interpolation_started = true; 
...
void Particle::Draw(ComponentCamera* active_camera, bool editor_camera)
{
	...
	if((particle_data->src_blending >= 0 && particle_data->src_blending <= 10) && (particle_data->dst_blending >= 0 && particle_data->dst_blending <= 10))
