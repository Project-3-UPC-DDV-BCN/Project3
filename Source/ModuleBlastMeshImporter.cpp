			...
			if (reader->getBoneCount() != 0)
			{
				...
				for (uint bone_index = 0; bone_index < reader->getBoneCount(); ++bone_index)
				{
					...
					for (uint i = 0; i < indices_count; i += 3) // CreateTriangles
					{
						...
						Vertex v1 = mesh->vertices_list[mesh->indices[i++]];
						Vertex v2 = mesh->vertices_list[mesh->indices[i++]];

						temp.a.Set(v0.GetPositionPtr());
						temp.b.Set(v1.GetPositionPtr());
						temp.c.Set(v2.GetPositionPtr());

						float3 edge0 = temp.b - temp.a;
						float3 edge1 = temp.c - temp.a;

						float2 deltaUV0(v1.GetUV().x - v0.GetUV().x, v1.GetUV().y - v0.GetUV().y);
						float2 deltaUV1(v2.GetUV().x - v0.GetUV().x, v2.GetUV().y - v0.GetUV().y);
						...
						tangent = tangent.float3::Normalized();
						...
						v0.SetTangent(tangent.x, tangent.y, tangent.z);
						v1.SetTangent(tangent.x, tangent.y, tangent.z);
						v2.SetTangent(tangent.x, tangent.y, tangent.z);
