		...
		if (reader)
		{
			...
			if (reader->getBoneCount() != 0)
			{
				...
				for (uint bone_index = 0; bone_index < reader->getBoneCount(); ++bone_index)
				{
					...
					//	float* v0 = &mesh->vertices_data[mesh->indices[i++] * 19];
					//	float* v1 = &mesh->vertices_data[mesh->indices[i++] * 19];
					//	float* v2 = &mesh->vertices_data[mesh->indices[i++] * 19];
					...
					//	float3 edge0 = temp.b - temp.a;
					//	float3 edge1 = temp.c - temp.a;

					//	float* uv0x = v0 + 4 * sizeof(float);
					//	float* uv0y = v0 + 5 * sizeof(float);

					//	float* uv1x = v1 + 4 * sizeof(float);
					//	float* uv1y = v1 + 5 * sizeof(float);

					//	float* uv2x = v2 + 4 * sizeof(float);
					//	float* uv2y = v2 + 5 * sizeof(float);

					//	float2 uv0(*uv0x, *uv0y);
					//	float2 uv1(*uv1x, *uv1y);
					//	float2 uv2(*uv2x, *uv2y);
					...
					//	t0.x = f * (deltaUV1.y * edge0.x - deltaUV0.y * edge1.x);
					//	t0.y = f * (deltaUV1.y * edge0.y - deltaUV0.y * edge1.y);
					//	t0.z = f * (deltaUV1.y * edge0.z - deltaUV0.y * edge1.z);

					//	b0.x = f * (-deltaUV1.x * edge0.x + deltaUV0.x * edge1.x);
					//	b0.y = f * (-deltaUV1.x * edge0.y + deltaUV0.x * edge1.y);
					//	b0.z = f * (-deltaUV1.x * edge0.z + deltaUV0.x * edge1.z);

					//	// Tangents 1
					//	float* t0x = v0 + 13 * sizeof(float);
					//	float* t0y = v0 + 14 * sizeof(float);
					//	float* t0z = v0 + 15 * sizeof(float);
					//	// Bitangents 1
					//	float* b0x = v0 + 16 * sizeof(float);
					//	float* b0y = v0 + 17 * sizeof(float);
					//	float* b0z = v0 + 18 * sizeof(float);

					//	// Tangents 2
					//	float* t1x = v1 + 13 * sizeof(float);
					//	float* t1y = v1 + 14 * sizeof(float);
					//	float* t1z = v1 + 15 * sizeof(float);
					//	// Bitangents 2
					//	float* b1x = v1 + 16 * sizeof(float);
					//	float* b1y = v1 + 17 * sizeof(float);
					//	float* b1z = v1 + 18 * sizeof(float);

					//	// Tangents 3
					//	float* t2x = v2 + 13 * sizeof(float);
					//	float* t2y = v2 + 14 * sizeof(float);
					//	float* t2z = v2 + 15 * sizeof(float);
					//	// Bitangents 3
					//	float* b2x = v2 + 16 * sizeof(float);
					//	float* b2y = v2 + 17 * sizeof(float);
					//	float* b2z = v2 + 18 * sizeof(float);
